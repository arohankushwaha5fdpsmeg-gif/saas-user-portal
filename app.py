import os,secrets,datetime as dt,requests as r,time,stripe
...
E_URL="https://ai-backend-engine.onrender.com" 
...
class User(UserMixin,db.Model):
    ...
    tokens=db.Column(db.Integer,default=15);is_premium=db.Column(db.Boolean,default=False);is_premium_plus=db.Column(db.Boolean,default=False);last_reset=db.Column(db.DateTime)

@lm.user_loader
def load_user(uid):return db.session.get(User,int(uid))
def v_tok(u):
    if u.is_premium or u.is_premium_plus:return
    if u.last_reset is None:u.last_reset=dt.datetime.utcnow();db.session.commit()
...
def register():
    if req.method=='POST':
        ...
        db.session.add(User(username=u,password=gh(p,method='scrypt'),last_reset=dt.datetime.utcnow()));db.session.commit()
...
def gen_code():
    ...
    p=req.json.get('p','') if req.json else '';cc=None
    for _ in range(2):
        try:
...
@login_required
def pay_premium():
    s=stripe.checkout.Session.create(line_items=[{'price_data':{'currency':'usd','product_data':{'name':'Premium Access'},'unit_amount':2500},'quantity':1}],mode='payment',success_url=req.host_url,cancel_url=req.host_url,customer_email=current_user.username)
    return red(s.url,code=33)

@app.route('/pay/plus',methods=['POST'])
@login_required
def pay_plus():
    s=stripe.checkout.Session.create(line_items=[{'price_data':{'currency':'usd','product_data':{'name':'Premium Plus Access'},'unit_amount':4900},'quantity':1}],mode='payment',success_url=req.host_url,cancel_url=req.host_url,customer_email=current_user.username)
    return red(s.url,code=33)
