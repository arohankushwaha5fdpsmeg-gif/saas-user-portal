...
app=Flask(__name__);app.config.update(SECRET_KEY=secrets.token_hex(8),SQLALCHEMY_DATABASE_URI=os.environ.get('DATABASE_URL','sqlite:///s.db'),SQLALCHEMY_TRACK_MODIFICATIONS=False);db=SQLAlchemy(app);lm=LoginManager(app);lm.login_view='login'
...
S="body{font-family:sans-serif;background:#0f172a;color:#fff;max-width:500px;margin:20px auto;padding:10px}.c{background:#1e293b;padding:15px;border-radius:6px;margin-bottom:10px}input,textarea{width:100%;padding:8px;margin:5px 0;background:#0f172a;color:#fff;border:1px solid #475569;border-radius:6px;box-sizing:border-box}button{width:100%;padding:10px;background:#38bdf8;border:none;color:#0f172a;font-weight:bold;border-radius:6px;cursor:pointer}.bp{background:#ec4899;color:#fff}"
H=f"""<!DOCTYPE html><html><head><style>{S}</style><title>AI</title></head><body><div class='c'><a href='/logout' style='color:#94a3b8;float:right;text-decoration:none;'>Logout</a><h2>AI Dashboard Pro 🚀</h2>
...
function g(){{const p=document.getElementById('p').value;if(!p.trim())return;document.getElementById('s').style.display='block';fetch('/gen',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{p:p}})}}).then(r=>r.json()).then(d=>{{document.getElementById('s').style.display='none';if(d.c){{document.getElementById('o').innerText=d.c;const s=document.getElementById('tc');if(s&&d.r!==undefined)s.innerText=d.r;}}else{{document.getElementById('o').innerText=d.message;}}}}).catch(e=>{{document.getElementById('s').style.display='none';document.getElementById('o').innerText='# Server connection lagging. Try again.';}});}}
