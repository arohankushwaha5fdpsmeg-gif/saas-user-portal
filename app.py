...
S = "body{font-family:sans-serif;background:#0f172a;color:#fff;max-width:500px;margin:20px auto;padding:10px}.c{background:#1e293b;padding:15px;border-radius:6px;margin-bottom:10px}input,textarea{width:100%;padding:8px;margin:5px 0;background:#0f172a;color:#fff;border:1px solid #475569;border-radius:6px;box-sizing:border-box}button{width:100%;padding:10px;background:#38bdf8;border:none;color:#0f172a;font-weight:bold;border-radius:6px;cursor:pointer;margin-top:5px}.btn-plus{background:#ec4899;color:#fff}"

H = f"""<!DOCTYPE html><html><head><style>{S}</style><title>AI Studio</title></head><body>
<div class='c'><a href='/logout' style='color:#94a3b8;float:right;text-decoration:none;'>Logout</a><h2>AI Dashboard Pro 🚀</h2>
<p>User: <b>{{{{current_user.username}}}}</b> | Tier: <span style='color:#38bdf8;font-weight:bold;'>{{%if current_user.is_premium_plus%}}👑 Premium Plus{{%elif current_user.is_premium%}}💎 Premium{{%else%}}<span id='tc'>{{{{current_user.tokens}}}}</span> / 15 Free Tokens{{%endif%}}</span></p>
<div style='display:flex;gap:10px;'>
<form action='/pay/premium' method='POST' style='flex:1;'><button type='submit' style='background:#7c3aed;color:#fff;'>Get Premium</button></form>
<form action='/pay/plus' method='POST' style='flex:1;'><button type='submit' class='btn-plus'>Get Premium Plus 👑</button></form>
</div>
{{%if not current_user.is_premium and not current_user.is_premium_plus and current_user.tokens <= 0%}}
<div style='background:#ef4444;padding:10px;border-radius:6px;text-align:center;margin-top:10px;'>⚠️ Tokens exhausted. Resets in 25 hours or unlock with a plan above.</div>
{{%endif%}}</div>
<div class='c'><textarea id='p' placeholder='Write any code or prompt...'></textarea><button onclick='g()'>Run Deep Generation ✨</button>
<p id='s' style='color:#fbbf24;display:none;text-align:center;font-weight:bold;'>🤖 Processing logic arrays... Please wait.</p></div>
<div class='c'><h3>Output:</h3><pre id='o' style='background:#020617;padding:12px;color:#34d399;overflow-x:auto;border-radius:6px;white-space:pre-wrap;'># Code will stay visible here safely without resetting...</pre>
<button style='background:#10b981;color:#fff;width:auto;' onclick='cp()'>Copy Code 📋</button></div>
<div class='c'><h3>Feedback</h3><textarea id='f' placeholder='Report bugs directly...'></textarea><button onclick='fb()'>Submit</button></div>
<script>
function g(){{const p=document.getElementById('p').value;if(!p.trim())return;document.getElementById('s').style.display='block';
fetch('/gen',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{p:p}})}}).then(r=>r.json()).then(d=>{{
document.getElementById('s').style.display='none';if(d.c){{document.getElementById('o').innerText=d.c;const s=document.getElementById('tc');if(s&&d.r!==undefined)s.innerText=d.r;}}else{{document.getElementById('o').innerText=d.message;}}}}).catch(e Dino=>{{document.getElementById('s').style.display='none';document.getElementById('o').innerText='# Server connection lagging. Try clicking generation button again.';}});}}
function cp(){{navigator.clipboard.writeText(document.getElementById('o').innerText);alert('Copied to clipboard!');}}
...
class Hist(db.Model):
    id = db.Column(db.Integer, primary_key=True); user_id = db.Column(db.Integer, nullable=False); prompt = db.Column(db.Text, nullable=False); response = db.Column(db.Text, nullable=False)

class Feedback(db.Model):
    id = db.Column(db.Integer, primary_key=True); user_id = db.Column(db.Integer, nullable=False); message = db.Column(db.Text, nullable=False)
...
def v_tok(u):
    ...
    if (dt.datetime.utcnow() - u.last_reset).total_seconds() / 3600.0 >= 25.0 and u.tokens < 15:
        u.tokens = 15; u.last_reset = dt.datetime.utcnow(); db.session.commit()
...
def gen_code():
    ...
    if current_user.tokens <= 0 and not current_user.is_premium and not current_user.is_premium_plus:
        return jsonify({'message': 'Expired tokens. Please upgrade.'}), 402
...
@login_required
def save_fb():
    db.session.add(Feedback(user_id=current_user.id, message=req.json.get('m', ''))); db.session.commit()
...
def webhook():
    ...
    if payload and payload.get('type') == 'checkout.session.completed':
        session_obj = payload['data']['object']
        email = session_obj.get('customer_details', {}).get('email')
        item_name = session_obj.get('line_items', [{}])[0].get('description', '')
        
        u = User.query.filter_by(username=email).first()
        if u:
            if "Plus" in item_name or session_obj.get('amount_total') == 4900:
                u.is_premium_plus = True
            else:
                u.is_premium = True
            db.session.commit()
